...
{
    internal class PipelineBinaryWriter
    {
        ...
        {
            _writer.GetSpan(sizeof(byte))[0] = val;
            _writer.Advance(sizeof(byte));
        }

        public void Write(ReadOnlySpan<byte> val)
        {
            var bytesLeft = val.Length;
            while (bytesLeft > 0)
            {
                var memory = _writer.GetMemory(bytesLeft);
        ...
        private void SerializeSingleBigEndian(float val, Span<byte> memory)
        {
            ...
            var result = MemoryMarshal.Cast<float, byte>(bytes);
            result.Reverse();
