...
namespace MongoDB.Client
{
    public class MongoSession : IAsyncDisposable
    {
        ...
        public MongoSession(Connection connection)
        {
        ...
        public async Task<MongoConnectionInfo?> SayHelloAsync(CancellationToken cancellationToken = default)
        {
            ...
            if (_shutdownToken.IsCancellationRequested == false)
            {
                if (protocol is not null)
                {
                    ...
                    SerializersMap.TryGetSerializer<MongoConnectionInfo>(out var serializer);
                    var bodyReader = new ReplyBodyReader<MongoConnectionInfo>(serializer);
                    return await ReadAsyncInternal(bodyReader, _shutdownToken.Token) as MongoConnectionInfo;
                }

                return ThrowHelper.ConnectionException<MongoConnectionInfo>(_connection.RemoteEndPoint!);
            }
            return ThrowHelper.ObjectDisposedException<MongoConnectionInfo>(nameof(Channel));
