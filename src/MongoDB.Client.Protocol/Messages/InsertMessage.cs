...
namespace MongoDB.Client.Protocol.Messages
{
    ...
    {
        public InsertMessage(int requestNumber, BsonDocument document)
            : this(requestNumber, Opcode.OpMsg, false, false, PayloadType.Type0, document)
        {
        }

        public InsertMessage(int requestNumber, Opcode opcode, bool moreToCome, bool exhaustAllowed, PayloadType payloadType, BsonDocument document)
        {
